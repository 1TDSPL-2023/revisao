...
let nomesCursos = cursos.map((cursos) => cursos.nome);
console.log(nomesCursos);


//iterando o array com foreach
console.log("\nIterando o array com foreach");
let nomesCursos2 = cursos.forEach((curso) => console.log(curso.nome));
...
function validarUsuario(nmUser, senhaUser){
    ...
    let usuarioLogado = usuarios.filter((usuario) => {
        return usuario.nome == nmUser.value && usuario.senha == senhaUser.value;
    });
    //Imprimir o objeto do usuário logado
...
//Exemplo de SPREAD (ele espalha as propriedades dentro de outra lista )
const curso2 = [0];
